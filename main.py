...
# # 카테고리 데이터 로드 및 전처리
# category_data = pd.read_csv('path/to/label_filtered.csv')
# category_data['filtered_label'] = category_data['filtered_label'].apply(ast.literal_eval)
# mlb = MultiLabelBinarizer()
# category_encoded = mlb.fit_transform(category_data['filtered_label'])
# category_encoded_df = pd.DataFrame(category_encoded, columns=mlb.classes_, index=category_data['restaurant_id'])

# # 카테고리 임베딩을 아이템 임베딩에 추가
# item_embeddings = np.random.randn(len(category_encoded_df), embedding_dim)  # 예를 들어 임베딩 차원
# item_embeddings += category_encoded_df.values  # 카테고리 벡터를 초기 임베딩에 추가
...
# Recommendation process for testing
model.eval()
user_num, item_num = train.shape
all_recs = []
for user in range(user_num):
    user_tensor = torch.tensor([user], device=device)
    scores = model.forward(user_tensor, None, None, None, test=True)  # Test mode
    top_items = scores.cpu().numpy().flatten()[:5]  # Top 5 items
    all_recs.append((user, top_items))

# Save recommendations to CSV
df = pd.DataFrame(all_recs, columns=['user', 'recommendations'])
df['recommendations'] = df['recommendations'].apply(lambda x: ','.join(map(str, x)))
output_path = path + 'recommendations.csv'
